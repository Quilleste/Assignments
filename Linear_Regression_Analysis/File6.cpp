...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	dichotomyCase(casefile, caseDichotomy);
		...
		MessageBox(NULL, "Ошибка открытия файла.", "Error", MB_OK);
	}
	return 0;
}
//---------------------------------------------------------------------------
...
void koefPierson(std::vector<patient> casefile, float matrix[8][8], int matrixSt[][8]) {
	...
	double Stkr = 1.975; //критерий стьюдента при заданом кол-ве данных и вероятности 0,05
	...
	//критерий значимости коеф. корреляции пирсона
	...
	std::cout << "Критерий Стьюдента при вероятности 0.05: " << Stkr << std::endl;
	std::cout << "----------------------------------------------" << std::endl;

	std::cout << "Матрица коэффициентов корреляции: " << std::endl << std::endl << std::setprecision(4);
	for(int i = 0; i < 8; i++) {
		for(int k = 0; k < 8; k++) {
	...
	std::cout << "Матрица значимостей коэффициентов корреляции: " << std::endl << std::endl;
	for(int i = 0; i < 8; i++) {
		for(int k = 0; k < 8; k++) {
